...
function getCompletedPrayerCount(prayers, userId, date) {
  return prayers.filter(
    (record) =>
      record.userId === userId && record.date === date && record.status === 'done',
  ).length
}
...
function hasLog(logs, userId, type, dates) {
  ...
  return logs.some(
    (record) =>
      record.userId === userId && record.type === type && dateSet.has(record.date),
  )
}
...
async function applyQuestProgress(userId, templates, type, period, progressByKey) {
  ...
  const records = templates.map((template) => {
    const existingRecord = existingQuests.find(
      (quest) =>
        quest.userId === userId &&
        quest.type === type &&
        quest.period === period &&
        quest.questKey === template.questKey,
    )
    const progress = Math.min(
      template.target,
      Number(progressByKey[template.questKey]) || 0,
    )
...
export async function refreshQuestData(userId) {
  ...
  const [prayers, logs] = await Promise.all([
    getAllFromStore('prayers'),
    getAllFromStore('ibadah_log'),
  ])

  const dailyQuests = await applyQuestProgress(
    userId,
    DAILY_QUESTS,
    'daily',
    todayKey,
  ...
  const weeklyQuests = await applyQuestProgress(
    userId,
    WEEKLY_QUESTS,
    'weekly',
    weekKey,
    {
      ...
      memorize_short_surah: hasLog(logs, userId, 'memorize_short_surah', weekDates)
        ? 1
        : 0,
    },
  )
...
export async function addSingleIbadahLog(userId, type) {
  ...
  const existingLog = await requestToPromise(
    db.transaction('ibadah_log', 'readonly').objectStore('ibadah_log').get(
      `${userId}:${type}:${date}`,
    ),
  )
