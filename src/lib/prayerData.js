...
import { createPrayerTimingState, getFirstPrayerFromSchedule, getPrayerScheduleForDate, getTodayPrayerSchedule } from "./prayerTimeData";
...
function getCompletedDateSet(records) {
  ...
  return new Set([...dates.entries()].filter(([, prayers]) => PRAYER_NAMES.every((prayer) => prayers.has(prayer))).map(([date]) => date));
}
...
async function getUserPrayerRecords(userId) {
  ...
  const records = await requestToPromise(db.transaction("prayers", "readonly").objectStore("prayers").getAll());
...
export async function getPrayerDashboardData(user, settings, now = new Date()) {
  ...
  const todayRecords = await Promise.all(PRAYER_NAMES.map((prayer) => requestToPromise(prayerStore.get(getPrayerId(user.id, todayKey, prayer)))));
  ...
  const completedCount = normalizedRecords.filter((record) => record.status === "done").length;
  const todayXP = normalizedRecords.reduce((total, record) => total + (record.xpEarned || 0), 0);
  ...
  const nextPrayer = timingState.nextPrayer || getFirstPrayerFromSchedule(await getPrayerScheduleForDate(user, settings, addDays(todayKey, 1)));
...
export async function setTodayPrayerStatus(userId, prayer, nextStatus) {
  ...
  const existingPrayer = await requestToPromise(db.transaction("prayers", "readonly").objectStore("prayers").get(prayerId));
  const existingUser = await requestToPromise(db.transaction("users", "readonly").objectStore("users").get(userId));
  ...
  const prayerRecord = {
    ...
    xpAwardedAt: existingPrayer?.xpAwardedAt || (shouldAwardXP ? now : undefined),
    updatedAt: now,
  };
  ...
  const refreshedUser = await requestToPromise(db.transaction("users", "readonly").objectStore("users").get(userId));
