...
function getAchievementProgress({ prayers, logs, quests, userId, currentStreak }) {
  const completedPrayers = prayers.filter(
    (record) => record.userId === userId && record.status === 'done',
  )
  const totalAyat = logs
    .filter(
      (record) =>
        record.userId === userId &&
        (record.type === 'tilawah' || record.type === 'quran'),
    )
    .reduce(
      (total, record) =>
        total + (Number(record.ayatCount ?? record.amount) || 0),
      0,
    )
  const completedQuestCount = quests.filter(
    (quest) => quest.userId === userId && quest.isCompleted,
  ).length
  const subuhCount = completedPrayers.filter(
    (record) => record.prayer === 'subuh',
  ).length
...
export async function refreshAchievements(user) {
  const db = await initDB()
  const [prayers, logs, quests, existingAchievements] = await Promise.all([
    getAll('prayers'),
    getAll('ibadah_log'),
    getAll('quests'),
    getAll('achievements'),
  ...
  const unlockedByKey = new Set(
    existingAchievements
      .filter((achievement) => achievement.userId === user.id)
  ...
  const newlyUnlocked = ACHIEVEMENTS.filter(
    (achievement) =>
      progressByKey[achievement.badgeKey] >= achievement.target &&
      !unlockedByKey.has(achievement.badgeKey),
  )

  if (newlyUnlocked.length > 0) {
    const xpToAward = newlyUnlocked.reduce(
  ...
  const recordsByKey = new Map(
    achievements
      .filter((achievement) => achievement.userId === user.id)
      .map((achievement) => [achievement.badgeKey, achievement]),
  )

  return {
    achievements: ACHIEVEMENTS.map((template) =>
      toAchievementView(
        template,
        recordsByKey.get(template.badgeKey),
        progressByKey[template.badgeKey] || 0,
      ),
    ).sort((a, b) => {
      ...
      return ACHIEVEMENTS.findIndex((item) => item.badgeKey === a.badgeKey) -
        ACHIEVEMENTS.findIndex((item) => item.badgeKey === b.badgeKey)
    }),
