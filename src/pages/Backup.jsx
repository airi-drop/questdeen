...
import { Bell, Calculator, Camera, Download, MapPinned, Palette, RotateCcw, Upload, User } from "lucide-react";
import AppShell from "../components/AppShell";
import Card from "../components/Card";
import { downloadBackup, exportBackupData, importBackupData, resetLocalData, validateBackup } from "../lib/backupData";
import { getOrCreateActiveUser, getSettings, updateActiveUser, updateSettings } from "../lib/db";
import { CALCULATION_METHOD_OPTIONS, MADHAB_OPTIONS, getDefaultPrayerSettings, getReadableLocationLabel, requestAndStoreLocation, requestNotificationPermission, DEFAULT_LOCATION } from "../lib/prayerTimeData";

const REMINDER_OFFSETS = [5, 10, 15, 30];

function Backup({ activePage = "backup", isDarkMode, onNavigate, onResetLocalData, onThemeChange, onToggleDarkMode, themeMode = "light" }) {
  ...
  const refreshLocalState = async () => {
    const [activeUser, storedSettings] = await Promise.all([getOrCreateActiveUser(), getSettings()]);
  ...
  const handleSaveProfile = async () => {
    ...
    if (name.length > 30) {
      showError("Nama maksimal 30 karakter.");
      return;
    }
  ...
  const handleProfileImageChange = async (event) => {
    ...
    try {
      ...
      setUser(savedUser);
      showMessage("Foto profil tersimpan.");

      if (profileImageInputRef.current) {
        profileImageInputRef.current.value = "";
      }
    } catch (error) {
      showError("Gagal menyimpan foto profil.");
  ...
  const handleImport = async (event) => {
    ...
    try {
      ...
      const confirmed = window.confirm("Importing this backup will replace local QuestDeen data. Continue?");
  ...
  const handleReset = async () => {
    const confirmed = window.confirm("Reset all local QuestDeen data on this device? This cannot be undone.");
  ...
  return (
    <AppShell activePage={activePage} isDarkMode={isDarkMode} onNavigate={onNavigate} onResetLocalData={onResetLocalData} onToggleDarkMode={onToggleDarkMode} user={user}>
      <section className="relative min-w-0 overflow-hidden rounded-[2rem] border border-[#d6cbb6] bg-[#0f3d2e] p-6 text-white shadow-[0_18px_45px_rgba(44,35,19,0.14)] dark:border-white/10 dark:bg-[#10241d] sm:p-7">
        <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#d9a441] dark:text-[#e6b84a]">Settings</p>
        <h1 className="mt-3 break-words text-[clamp(1.75rem,3vw,2.75rem)] font-bold leading-tight tracking-tight">Profile & Preferences</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-[#dce8dd] sm:text-base">Manage prayer calculations, app preferences, local backup, and PWA data on this device.</p>
      </section>

      {message && <p className="rounded-2xl border border-[#c7dbc0] bg-[#dde8d2]/70 px-4 py-3 text-sm font-semibold text-[#1f6f50] dark:border-[#2ddfa3]/20 dark:bg-[#2ddfa3]/10 dark:text-[#2ddfa3]">{message}</p>}
      {errorMessage && <p className="rounded-2xl border border-[#e7c8bd] bg-[#f8ece8] px-4 py-3 text-sm font-semibold text-[#9a3d2f] dark:border-[#c56b5c]/30 dark:bg-[#9a3d2f]/10 dark:text-[#ffb4a8]">{errorMessage}</p>}

      <div className="grid gap-5 lg:grid-cols-3">
        <Card>
          <User className="mb-4 text-[#1f6f50] dark:text-[#2ddfa3]" />
          <h2 className="text-xl font-bold text-[#17352b] dark:text-[#f7f3e8]">Profile</h2>
          <div className="mt-5 flex min-w-0 items-center gap-4">
            {user?.profileImage ? (
              <img alt="" className="size-20 shrink-0 rounded-2xl object-cover shadow-sm" src={user.profileImage} />
            ) : (
              <div className="grid size-20 shrink-0 place-items-center rounded-2xl bg-[#d9a441] text-3xl font-bold text-[#2d2408] dark:bg-[#e6b84a]">{(user?.avatarInitial || user?.name || "A").slice(0, 1).toUpperCase()}</div>
            )}
            <div className="min-w-0">
              <p className="truncate text-lg font-bold text-[#17352b] dark:text-[#f7f3e8]">{user?.name || "Guest"}</p>
              <p className="mt-1 text-sm text-[#6f7e76] dark:text-[#a7b8b2]">
                Level {user?.level || 1} · {user?.totalXP || 0} XP
              </p>
              <p className="mt-1 text-sm text-[#6f7e76] dark:text-[#a7b8b2]">Current streak: {user?.currentStreak || 0} days</p>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <input accept="image/*" className="hidden" onChange={handleProfileImageChange} ref={profileImageInputRef} type="file" />
          </div>
        </Card>

        <Card className="lg:col-span-2">
          <Palette className="mb-4 text-[#1f6f50] dark:text-[#2ddfa3]" />
          <h2 className="text-xl font-bold text-[#17352b] dark:text-[#f7f3e8]">App preferences</h2>
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <label className="text-sm font-semibold text-[#17352b] dark:text-[#f7f3e8]">
              ...
              <select
                className="mt-2 w-full rounded-xl border border-[#e4dccb] bg-white/70 px-3 py-2 text-sm text-[#17352b] outline-none transition focus:border-[#1f6f50] focus:ring-2 focus:ring-[#1f6f50]/20 dark:border-white/10 dark:bg-[#071a16] dark:text-[#f7f3e8]"
                onChange={(event) => handleAppSettingChange({ language: event.target.value })}
          ...
          <p className="mt-4 text-sm text-[#6f7e76] dark:text-[#a7b8b2]">Theme and language preferences are saved to IndexedDB and applied across future visits.</p>
        </Card>
        ...
        <Card className="lg:col-span-3">
          <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <h2 className="text-xl font-bold text-[#17352b] dark:text-[#f7f3e8]">Prayer settings</h2>
              <p className="mt-2 text-sm text-[#6f7e76] dark:text-[#a7b8b2]">Calculation, madhab, location, and reminder preferences are stored locally.</p>
            </div>
            <button
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <label className="min-w-0 text-sm font-semibold text-[#17352b] dark:text-[#f7f3e8]">
              ...
              <select
                className="mt-2 w-full rounded-xl border border-[#e4dccb] bg-white/70 px-3 py-2 text-sm text-[#17352b] outline-none transition focus:border-[#1f6f50] focus:ring-2 focus:ring-[#1f6f50]/20 dark:border-white/10 dark:bg-[#071a16] dark:text-[#f7f3e8]"
                onChange={(event) => handlePrayerSettingChange({ calculationMethod: event.target.value })}
            ...
            <label className="min-w-0 text-sm font-semibold text-[#17352b] dark:text-[#f7f3e8]">
              ...
              <select
                className="mt-2 w-full rounded-xl border border-[#e4dccb] bg-white/70 px-3 py-2 text-sm text-[#17352b] outline-none transition focus:border-[#1f6f50] focus:ring-2 focus:ring-[#1f6f50]/20 dark:border-white/10 dark:bg-[#071a16] dark:text-[#f7f3e8]"
                onChange={(event) => handlePrayerSettingChange({ madhab: event.target.value })}
            ...
            <div className="rounded-2xl border border-[#e4dccb] bg-white/50 p-4 dark:border-white/10 dark:bg-white/[0.04]">
              <p className="text-sm font-bold text-[#17352b] dark:text-[#f7f3e8]">Saved location</p>
              <p className="mt-2 text-sm font-semibold text-[#17352b] dark:text-[#f7f3e8]">{locationLabel}</p>
              <p className="mt-1 text-xs leading-5 text-[#6f7e76] dark:text-[#a7b8b2]">{location.source === "default" ? "default location" : "browser location"}</p>
            </div>
          </div>
        ...
        <Card>
          ...
          <h2 className="text-xl font-bold text-[#17352b] dark:text-[#f7f3e8]">Export backup</h2>
          <p className="mt-2 text-sm text-[#6f7e76] dark:text-[#a7b8b2]">Download all QuestDeen local data as a JSON backup.</p>
          <button className="mt-5 rounded-xl bg-[#1f6f50] px-4 py-2 text-sm font-bold text-white transition hover:-translate-y-0.5 dark:bg-[#2ddfa3] dark:text-[#071a16]" onClick={handleExport} type="button">
            Export Data
          </button>
        </Card>
        ...
        <Card>
          ...
          <h2 className="text-xl font-bold text-[#17352b] dark:text-[#f7f3e8]">Import backup</h2>
          <p className="mt-2 text-sm text-[#6f7e76] dark:text-[#a7b8b2]">Restore a QuestDeen JSON backup after confirmation.</p>
          <input accept="application/json" className="mt-5 w-full rounded-xl border border-[#e4dccb] bg-white/70 px-3 py-2 text-sm dark:border-white/10 dark:bg-white/[0.04]" onChange={handleImport} ref={fileInputRef} type="file" />
        </Card>

        <Card>
          <RotateCcw className="mb-4 text-[#9a3d2f]" />
          <h2 className="text-xl font-bold text-[#17352b] dark:text-[#f7f3e8]">Danger zone</h2>
          <p className="mt-2 text-sm text-[#6f7e76] dark:text-[#a7b8b2]">Clear QuestDeen local data only, then recreate default settings.</p>
          <button className="mt-5 rounded-xl bg-[#9a3d2f] px-4 py-2 text-sm font-bold text-white transition hover:-translate-y-0.5" onClick={handleReset} type="button">
            Reset Local Data
          </button>
        </Card>
      </div>
