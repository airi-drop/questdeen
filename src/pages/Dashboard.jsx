...
import { getPrayerDashboardData, setTodayPrayerStatus } from "../lib/prayerData";
import { requestAndStoreLocation, schedulePrayerNotifications } from "../lib/prayerTimeData";
import { addQuranAyatLog, addSingleIbadahLog, refreshQuestData } from "../lib/questData";

function Dashboard({ activePage = "dashboard", isDarkMode, onNavigate, onResetLocalData, onToggleDarkMode }) {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    const todayXP = prayerData.todayXP + questData.dailyQuests.reduce((total, quest) => total + (quest.xpEarned || 0), 0);
  ...
  useEffect(() => {
    const refreshEnabled = dashboardData.settings?.prayerSettings?.autoLocationRefresh;
  ...
  const showProgressToasts = useCallback(
    (previousData, nextData) => {
      const previousQuestKeys = new Set([...previousData.dailyQuests, ...previousData.weeklyQuests].filter((quest) => quest.isCompleted).map((quest) => quest.id));
      const completedQuests = [...nextData.dailyQuests, ...nextData.weeklyQuests].filter((quest) => quest.isCompleted && !previousQuestKeys.has(quest.id));
      const previousAchievementKeys = new Set(previousData.achievements.filter((achievement) => achievement.isUnlocked).map((achievement) => achievement.badgeKey));
      const unlockedAchievements = nextData.achievements.filter((achievement) => achievement.isUnlocked && !previousAchievementKeys.has(achievement.badgeKey));

      completedQuests.forEach((quest) => {
        addToast("Quest completed", `${quest.title} +${quest.xpEarned || quest.xpReward} XP`);
      });
      unlockedAchievements.forEach((achievement) => {
        addToast("Achievement unlocked", `${achievement.title} +${achievement.xpEarned || achievement.xpReward || 0} XP`);
      });
    },
    [addToast],
  );
  ...
  const handleTogglePrayer = async (prayer) => {
    ...
    try {
      const user = await setTodayPrayerStatus(dashboardData.user.id, prayer.prayer, nextStatus);
      ...
      if (nextStatus === "done" && nextData.prayers.filter((item) => item.isDone).length === 5 && dashboardData.prayers.filter((item) => item.isDone).length < 5) {
  ...
  const handleAddSingleLog = async (type) => {
    ...
    const feedbackByType = {
      dzikir: result.created ? "Dzikir sore ditandai selesai." : "Dzikir sore sudah tercatat hari ini.",
      sedekah: result.created ? "Sedekah ditandai selesai." : "Sedekah sudah tercatat hari ini.",
      memorize_short_surah: result.created ? "Hafalan surah pendek ditandai selesai." : "Hafalan surah pendek sudah tercatat hari ini.",
    };
  ...
  const weeklyQuestActions = {
    memorize_short_surah: (
      <button className="rounded-xl bg-[#1f6f50] px-4 py-2 text-sm font-bold text-white transition hover:-translate-y-0.5 dark:bg-[#2ddfa3] dark:text-[#071a16]" onClick={() => handleAddSingleLog("memorize_short_surah")} type="button">
        Mark memorized
      </button>
    ),
  };
  ...
  return (
    <AppShell activePage={activePage} isDarkMode={isDarkMode} onNavigate={onNavigate} onResetLocalData={onResetLocalData} onToggleDarkMode={onToggleDarkMode} user={dashboardData.user}>
      {isLoading ? (
        <div className="flex min-h-screen items-center justify-center">
          <div className="text-center">
            <div className="mb-4 inline-block">
              <div className="h-12 w-12 animate-spin rounded-full border-4 border-[#e4dccb] border-t-[#1f6f50] dark:border-white/10 dark:border-t-[#2ddfa3]" />
            </div>
            <p className="text-sm font-semibold text-[#6f7e76] dark:text-[#a7b8b2]">Loading your dashboard...</p>
          </div>
        </div>
      ) : (
        <>
          <section className="relative min-w-0 overflow-hidden rounded-[2rem] border border-[#d6cbb6] bg-[#0f3d2e] p-6 text-white shadow-[0_18px_45px_rgba(44,35,19,0.14)] dark:border-white/10 dark:bg-[#10241d] sm:p-7">
            ...
            <div className="relative max-w-3xl">
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#d9a441] dark:text-[#e6b84a]">Level {dashboardData.user?.level || 1}</p>
              <h2 className="mt-3 break-words text-[clamp(1.75rem,3vw,2.75rem)] font-bold leading-tight tracking-tight">Welcome back, {dashboardData.user?.name || "Worshiper"}.</h2>
              <p className="mt-3 max-w-2xl text-sm leading-6 text-[#dce8dd] sm:text-base">Keep your momentum with prayer tracking, quests, and progress that updates from your local device.</p>
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                <div className="rounded-3xl border border-[#c7dbc0] bg-white/10 px-4 py-3 text-sm text-[#dce8dd] dark:border-white/10 dark:bg-white/5">
                  <p className="font-semibold text-[#f7f3e8]">Streak</p>
                  <p className="mt-1 text-xl font-bold text-white">{dashboardData.user?.currentStreak || 0} days</p>
                </div>
                <div className="rounded-3xl border border-[#c7dbc0] bg-white/10 px-4 py-3 text-sm text-[#dce8dd] dark:border-white/10 dark:bg-white/5">
                  <p className="font-semibold text-[#f7f3e8]">Total XP</p>
                  <p className="mt-1 text-xl font-bold text-white">{dashboardData.user?.totalXP || 0}</p>
                </div>
                <div className="rounded-3xl border border-[#c7dbc0] bg-white/10 px-4 py-3 text-sm text-[#dce8dd] dark:border-white/10 dark:bg-white/5">
                  <p className="font-semibold text-[#f7f3e8]">Prayers done</p>
                  <p className="mt-1 text-xl font-bold text-white">{dashboardData.prayers.filter((prayer) => prayer.isDone).length}/5</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <QuickIbadahLogCard
            ...
            dzikirDone={dashboardData.dailyQuests.find((quest) => quest.questKey === "evening_dhikr")?.isCompleted}
            ...
            sedekahDone={dashboardData.weeklyQuests.find((quest) => quest.questKey === "weekly_sedekah")?.isCompleted}
          />

          <div className="mt-5 grid min-w-0 gap-5 lg:grid-cols-2">
            <QuestSection title="Daily quests" quests={dashboardData.dailyQuests} />
            <QuestSection actionControls={weeklyQuestActions} title="Weekly quests" quests={dashboardData.weeklyQuests} />
          </div>
