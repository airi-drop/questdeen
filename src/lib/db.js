...
export async function getActiveUser() {
  ...
  const settings = await promisifyRequest(
    transaction.objectStore('settings').get(SETTINGS_ID),
  )
...
export async function getOrCreateActiveUser() {
  ...
  const db = await initDB()
  const existingDefaultUser = await promisifyRequest(
    db.transaction('users', 'readonly').objectStore('users').get(DEFAULT_USER_ID),
  )
