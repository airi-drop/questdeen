...
function getCurrentMonthDateKeys(date = new Date()) {
  ...
  return Array.from({ length: daysInMonth }, (_, index) =>
    getTodayKey(new Date(year, month, index + 1)),
  )
}
...
export async function getStatisticsData() {
  ...
  const [prayers, quests, logs] = await Promise.all([
    getAll('prayers'),
    getAll('quests'),
    getAll('ibadah_log'),
  ])
  ...
  const totalCompletedPrayers = prayers.filter(
    (record) => record.userId === user.id && record.status === 'done',
  ).length
  const totalQuranAyat = getAyatTotal(logs, user.id, logs.map((log) => log.date))
  const completedQuests = quests.filter(
    (quest) => quest.userId === user.id && quest.isCompleted,
  )
  ...
  const dailyCompletedThisWeek = completedQuests.filter(
    (quest) => quest.type === 'daily' && weekKeys.includes(quest.period),
  ).length
