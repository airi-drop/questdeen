...
function getDistanceKm(firstLocation, secondLocation) {
  if (typeof firstLocation?.latitude !== "number" || typeof firstLocation?.longitude !== "number" || typeof secondLocation?.latitude !== "number" || typeof secondLocation?.longitude !== "number") {
  ...
  const haversine = Math.sin(latDistance / 2) ** 2 + Math.cos(firstLat) * Math.cos(secondLat) * Math.sin(lonDistance / 2) ** 2;
...
function createLocationLabel({ city, district, province }) {
  ...
  const normalizedDistrict = district && !/^kecamatan\s/i.test(district) ? `Kecamatan ${district}` : district;
...
async function reverseGeocodeLocation(location) {
  ...
  const city = getAddressPart(address, ["city", "town", "municipality", "city_district"]);
  const district = getAddressPart(address, ["suburb", "district", "subdistrict", "village", "neighbourhood"]);
...
function createCalculationParams(prayerSettings = {}) {
  ...
  params.madhab = prayerSettings.madhab === "hanafi" ? Madhab.Hanafi : Madhab.Shafi;

  return params;
}

function getLocation(user) {
  return typeof user?.location?.latitude === "number" && typeof user?.location?.longitude === "number" ? user.location : DEFAULT_LOCATION;
}
...
function serializeTimes(prayerTimes) {
  return Object.fromEntries(Object.entries(PRAYER_TIME_KEYS).map(([prayer, adhanKey]) => [prayer, prayerTimes[adhanKey].toISOString()]));
}
...
async function getCachedSchedule(userId, date) {
  ...
  return requestToPromise(db.transaction("prayer_schedules", "readonly").objectStore("prayer_schedules").get(getCacheId(userId, date)));
}
...
export function createPrayerTimingState({ prayers, schedule, now = new Date() }) {
  ...
  const nextPrayer = timeline.find((prayer) => prayer.startsAt.getTime() > now.getTime()) || null;
  const activeIndex = timeline.findIndex((prayer, index) => {
    const nextStart = timeline[index + 1]?.startsAt;

    return prayer.startsAt.getTime() <= now.getTime() && (!nextStart || now.getTime() < nextStart.getTime());
  });
...
export async function requestAndStoreLocation({ forceReverseGeocode = false } = {}) {
  ...
  const db = await initDB();
  const settings = await requestToPromise(db.transaction("settings", "readonly").objectStore("settings").get(SETTINGS_ID));
  const user = await requestToPromise(db.transaction("users", "readonly").objectStore("users").get(settings?.activeUserId));
  ...
  const shouldReverseGeocode = forceReverseGeocode || !hasFreshReverseGeocode(user.location) || distanceKm >= LOCATION_REFRESH_DISTANCE_KM;
...
export function schedulePrayerNotifications({ prayers, settings, now = new Date() }) {
  ...
  prayers.forEach((prayer) => {
    ...
    const beforeAt = new Date(startsAt.getTime() - prayerSettings.notificationOffsetMinutes * 60000);
    ...
    if (prayerSettings.notifyBeforePrayer && beforeDelay > 0) {
      notificationTimers.push(
        window.setTimeout(() => {
          createNotification("QuestDeen", `Waktu ${prayer.name} sebentar lagi.`);
        }, beforeDelay),
      );
    }
    ...
    if (prayerSettings.notifyAtPrayerTime && atDelay > 0) {
      notificationTimers.push(
        window.setTimeout(() => {
          createNotification("QuestDeen", `Saatnya menunaikan shalat ${prayer.name}.`);
        }, atDelay),
      );
    }
  });
